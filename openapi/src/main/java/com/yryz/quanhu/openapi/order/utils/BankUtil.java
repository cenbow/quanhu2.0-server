...
public class BankUtil {
    public static void main(String[] args) {
        String cardNumber = "6222023202032489998";// 卡号
        String name = getNameOfBank(cardNumber);
        System.out.println(name.split("·")[0].trim());
    }

    public static String getSimpleNameOfBank(String idCard) {
        String bankName = getNameOfBank(idCard);
        return bankName.split("·")[0].trim();
    }
     ...
     * @return
     */
    public static String getWxBankId(String idCard) {
        String simpleBankName = getSimpleNameOfBank(idCard);
        return WX_BANK_ID_MAPPING.get(simpleBankName);
    }
    ...
    private final static String[] bankBin = {"621098", "622150", "622151", "622181", "622188", "955100", "621095",
            ...
            "628267", "621392", "621481", "621310", "621396", "623251", "628351", "685800"};
    ...
    private static final String[] bankName = {"邮储银行·绿卡通", "邮储银行·绿卡银联标准卡", "邮储银行·绿卡银联标准卡", "邮储银行·绿卡专用卡", "邮储银行·绿卡银联标准卡",
            ...
            "兴业银行·兴业芯片白金卡", "兴业银行·兴业芯片钻石卡", "浦发银行·浦发JCB金卡", "浦发银行·浦发JCB白金卡", "浦发银行·信用卡VISA普通", "浦发银行·信用卡VISA金卡",
            "浦发银行·浦发银行VISA年青卡", "浦发银行·VISA白金信用卡", "浦发银行·浦发万事达白金卡", "浦发银行·浦发JCB普卡", "浦发银行·浦发万事达金卡",
            "浦发银行·浦发万事达普卡", "浦发银行·浦发单币卡", "浦发银行·浦发银联单币麦兜普卡", "浦发银行·东方轻松理财卡", "浦发银行·东方-轻松理财卡普卡",
            "浦发银行·东方轻松理财卡", "浦发银行·东方轻松理财智业金卡", "浦发银行·东方卡(银联卡)", "浦发银行·东方卡(银联卡)", "浦发银行·东方卡(银联卡)",
            "浦发银行·公务卡金卡", "浦发银行·公务卡普卡", "浦发银行·东方卡", "浦发银行·东方卡", "浦发银行·浦发单币卡", "浦发银行·浦发联名信用卡",
            "浦发银行·浦发银联白金卡", "浦发银行·轻松理财普卡", "浦发银行·移动联名卡", "浦发银行·轻松理财消贷易卡", "浦发银行·轻松理财普卡（复合卡）", "浦发银行·贷记卡",
            "浦发银行·贷记卡", "浦发银行·东方借记卡（复合卡）", "浦发银行·电子现金卡（IC卡）", "浦发银行·移动浦发联名卡", "浦发银行·东方-标准准贷记卡",
            "浦发银行·轻松理财金卡（复合卡）", "浦发银行·轻松理财白金卡（复合卡）", "浦发银行·轻松理财钻石卡（复合卡）", "浦发银行·东方卡", "恒丰银行·九州IC卡",
            ...
            "东营莱商村镇银行·绿洲卡", "建设银行·单位结算卡", "玉溪市商业银行·红塔卡", "广发银行·理财通"};

    //微信提现支持的17家银行的code
    private static final Map<String, String> WX_BANK_ID_MAPPING = new HashMap<>();

    static {
        WX_BANK_ID_MAPPING.put("工商银行", "1002");
        WX_BANK_ID_MAPPING.put("农业银行", "1005");
        WX_BANK_ID_MAPPING.put("中国银行", "1026");
        WX_BANK_ID_MAPPING.put("建设银行", "1003");
        WX_BANK_ID_MAPPING.put("招商银行", "1001");
        WX_BANK_ID_MAPPING.put("邮储银行", "1066");
        WX_BANK_ID_MAPPING.put("交通银行", "1020");
        WX_BANK_ID_MAPPING.put("浦发银行", "1004");
        WX_BANK_ID_MAPPING.put("民生银行", "1006");
        WX_BANK_ID_MAPPING.put("兴业银行", "1009");
        WX_BANK_ID_MAPPING.put("平安银行", "1010");
        WX_BANK_ID_MAPPING.put("中信银行", "1021");
        WX_BANK_ID_MAPPING.put("华夏银行", "1025");
        WX_BANK_ID_MAPPING.put("广发银行", "1027");
        WX_BANK_ID_MAPPING.put("光大银行", "1022");
        WX_BANK_ID_MAPPING.put("北京银行", "1032");
        WX_BANK_ID_MAPPING.put("宁波银行", "1056");
    }
