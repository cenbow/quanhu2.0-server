...
public class OnceRuleScoreServiceImpl extends BaseRuleScoreServiceImpl implements OnceRuleScoreService{
	...
	public boolean processStatus(String custId, String eventCode , ScoreEventInfo sei , double amount) {
		...
		String statusKey = EventUtil.getScoreStatusKey(custId, eventCode , ScoreTypeEnum.Once);
		//尽管状态机放行，这里仍先查询一次性积分事件状态表有没有记录过状态
		ScoreStatusOnce sso = scoreStatusOnceService.getByCode(custId, eventCode);
		...
		//1. 一次性积分状态记录
		Date now = new Date();
		sso = new ScoreStatusOnce(custId , eventCode);
		sso.setScoreFlag(1);
